...
for (method in c("poisson", "quasipoisson", "negbin", "mm1", "mm2")) {
  results1[[method]]<-data.frame(iter=seq(1,SIMNUM), real.coef=numeric(SIMNUM), est.coef=numeric(SIMNUM), stderr=numeric(SIMNUM), pval=numeric(SIMNUM))
...
for (i in seq(1,SIMNUM)) {
  #shuffle for regular
  sim.ogs$case = FALSE
  sim.ogs$case[sample(nrow(sim.ogs), TRUECT)]=TRUE
  ...
  #assign 
  results1[["poisson"]][i,]=c(i, sim.realcoef, coef(sim.pois)[2,c(1,2,4)])
  results1[["mm1"]][i,]=c(i, sim.realcoef, coef(sim.ml1)[2,c(1,2,4)])
  results1[["mm2"]][i,]=c(i, sim.realcoef, coef(sim.ml2)[2,c(1,2,4)])
  results1[["negbin"]][i,]=c(i, sim.realcoef, coef(sim.nb)[2,c(1,2,4)])
  results1[["quasipoisson"]][i,]=c(i, sim.realcoef, coef(sim.qp)[2,c(1,2,4)])
...
saveRDS(results1, file="perms.results1")
saveRDS(results1, file="perms.results2")